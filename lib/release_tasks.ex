...
#   def repos, do: Application.get_env(myapp(), :ecto_repos, [])
#
#   def create(), do: run(:create)
#   def migrate(), do: run(:migrate)
#   def seed(), do: run(:seed)
#
#   def run(type) do
#     me = myapp()
#
...
#     IO.inspect(repos())
#
#     repos().start_link(pool_size: 1)
#
#     case type do
#       :create  -> create()
#       :migrate -> migrate()
#       :seed    -> Enum.each(repos(), &run_seeds_for/1)
#     end
#
#     # Signal shutdown
#     IO.puts "Success!"
...
#   end
#
#   def create, do: Enum.each(repos(), &run_create_for/1)
#
#   defp run_create_for(repo) do
#     app = Keyword.get(repo.config, :otp_app)
#     IO.puts "Creating storage for #{app}"
